...
Citizen 100-class BiLSTM (CC BY-NC-SA 4.0). The earlier 50-class weights remain
...
function translateClip(video, landmarks, durationMs, quality):
  ...
  pred = onnxBiLSTM(features)                # softmax over 100 glosses
...
5. UI (`sign-studio.tsx`): **Dedicated model** vs **Gemini video** badge, plus **Model v2.1.3 (RL) 100-class** on the dedicated path, gloss + confidence, Replay voice on `english`.
...
anything outside the 100-gloss list use Gemini video. Long clips fail closed
