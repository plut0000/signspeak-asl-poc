...
1. Train offline → `models/asl-citizen-bilstm20/` (`*.pt`, `label_map.json`, exported `*.onnx`). Demo **v2.1.1** still loads the v2.1 `asl_citizen_bilstm20_rl.onnx` graph; routing now skips it for long clips.
