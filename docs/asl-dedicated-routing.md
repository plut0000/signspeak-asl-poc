...
Citizen 50-class BiLSTM (CC BY-NC-SA 4.0). The earlier 20-class weights remain
in `models/asl-citizen-bilstm20/`. The earlier WLASL TCN scaffold remains parked
...
function translateClip(video, landmarks, durationMs, quality):
  ...
  pred = onnxBiLSTM(features)                # softmax over 50 glosses
...
1. Train offline → `models/asl-citizen-bilstm50/` (`*.pt`, `label_map50.json`, exported `*.onnx`). Demo **v2.1.2.1** loads the v2.1.2 `asl_citizen_bilstm50_rl.onnx` graph; v2.1.1 routing still skips BiLSTM for long clips. Gemini video is silent (audio stripped) and lyric-first.
...
5. UI (`sign-studio.tsx`): **Dedicated model** vs **Gemini video** badge, plus **Model v2.1.2 (RL) 50-class** on the dedicated path, gloss + confidence, Replay voice on `english`.
...
anything outside the 50-gloss list use Gemini video. Long clips fail closed
